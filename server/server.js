// Freeze Faker multiplayer server: rooms by code + one authoritative match each.
// Authoritative for: NPC crowd, light cycle, accusation resolution, win/lose.
// Player characters are client-controlled and relayed. NPCs and human Fakers
// are broadcast as one indistinguishable entity list so the Watcher cannot
// tell humans from NPCs by metadata (hidden-role integrity).
...
class Match {
  constructor(room) {
    this.room = room;
    this.rng = makeRng((Date.now() >>> 0) ^ (uid * 2654435761));
    ...
    // Faker players get a server-side char (appearance + spawn); client controls it.
    ...
    // Stable entity order for snapshots: NPCs then faker chars.
  ...
  rosterFor() {
    // Appearance + initial pose for every entity, no kind leak.
  ...
  tick(dt) {
    ...
    // Gate check for fakers (positions relayed from clients).
  ...
  _end(winner, reason) {
    ...
    // reveal faker ids to everyone at the end
...
class Room {
  ...
  remove(id) {
    ...
    if (this.match && p.char) p.char.eliminated = true; // drop out of the crowd
...
const server = createServer({
  root: ROOT,
  onConnection(conn) {
    const player = { id: uid++, name: 'Player', conn, ready: false, role: null, room: null, char: null };
    conn.on('message', (raw) => {
...
function handle(player, m) {
  switch (m.t) {
    ...
    case 'role': { // host may reassign own/other roles minimally: toggle a target between faker/watcher
      const room = player.room; if (!room || room.hostId !== player.id || room.match) return;
      // ensure exactly one watcher: set target as watcher, others faker
    ...
    case 'state': { // faker relays their char state
